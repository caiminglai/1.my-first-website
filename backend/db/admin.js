const { prepare, exec } = require("./index");
...
function getDisciplines() {
  return prepare(`
    SELECT d.学科ID as id, d.学科名称 as name, d.学科颜色 as color, d.学科描述 as description, d.显示顺序 as display_order, COUNT(t.词条ID) AS term_count
    FROM 学科 d
...
function createDiscipline(key, name, color, description) {
  const maxOrder = prepare(
    "SELECT COALESCE(MAX(显示顺序), 0) as max FROM 学科",
  ).get().max;
  return prepare(
    "INSERT INTO 学科 (学科ID, 学科名称, 学科颜色, 学科描述, 显示顺序) VALUES (?, ?, ?, ?, ?)",
  ).run(key, name, color || "#6B7B5E", description || "", maxOrder + 1);
}
...
function updateDisciplineOrder(order) {
  ...
  try {
    order.forEach((id, index) => {
      prepare("UPDATE 学科 SET 显示顺序 = ? WHERE 学科ID = ?").run(
        index + 1,
        id,
      );
    });
...
function updateDiscipline(id, updates) {
  ...
  values.push(id);
  return prepare(
    `UPDATE 学科 SET ${fields.join(", ")} WHERE 学科ID = ?`,
  ).run(...values);
}

function deleteDiscipline(id) {
  return prepare("DELETE FROM 学科 WHERE 学科ID = ?").run(id);
}

function countTermsByDiscipline(disciplineId) {
...
function getAllTerms(discipline) {
  let sql =
    "SELECT 词条ID as id, 学科 as discipline, 名称 as name, 翻译 as translation, 本质 as essence, 提示 as tip, 热度 as hot FROM 词条";
  const params = [];
  if (discipline) {
    sql += " WHERE 学科 = ?";
    params.push(discipline);
...
function getDashboardStats() {
  ...
  const byDiscipline = prepare(`
    SELECT d.学科ID AS discipline, COUNT(t.词条ID) AS count
    FROM 学科 d
    LEFT JOIN 词条 t ON t.学科 = d.学科ID
    GROUP BY d.学科ID
    ORDER BY d.显示顺序, d.学科ID
