...
const { callLLM, callLLMStream, getSupportedProviders } = require("./ai");
...
async function generateTerm({ provider, apiKey, baseUrl, model, name, discipline }) {
  if (!name?.trim()) {
    throw new Error("MISSING_TERM_NAME");
  }
  if (!discipline?.trim()) {
    throw new Error("MISSING_DISCIPLINE");
  }
  if (!apiKey) {
    throw new Error("MISSING_API_KEY");
  }
...
async function generateComparison({ provider, apiKey, baseUrl, model, nameA, nameB }) {
  if (!nameA?.trim()) {
    throw new Error("MISSING_NAME_A");
  }
  if (!nameB?.trim()) {
    throw new Error("MISSING_NAME_B");
  }
  if (!apiKey) {
    throw new Error("MISSING_API_KEY");
  }
