...
function getAllJobsWithDetails(categoryId) {
  ...
  const allStages = prepare(
    `SELECT 阶段ID as stage_id, 岗位ID as job_id, 阶段序号 as stage_number, 阶段标题 as title, 阶段副标题 as subtitle, 显示顺序 as display_order FROM 岗位学习阶段 WHERE 岗位ID IN (${jobIdPlaceholders}) ORDER BY 显示顺序`,
  ).all(...jobIds);

  const stageIds = allStages.map((s) => s.stage_id);
  let allSkills = [];
  if (stageIds.length > 0) {
    const stageIdPlaceholders = stageIds.map(() => "?").join(",");
    allSkills = prepare(
      `SELECT 技能ID as skill_id, 阶段ID as stage_id, 技能名称 as name, 技能描述 as description, 显示顺序 as display_order FROM 岗位技能 WHERE 阶段ID IN (${stageIdPlaceholders}) ORDER BY 显示顺序`,
    ).all(...stageIds);
  }
  const allResources = prepare(
    `SELECT 资源ID as resource_id, 岗位ID as job_id, 资源类型 as type, 资源标题 as title, 资源描述 as description, 资源链接 as link, 显示顺序 as display_order FROM 岗位学习资源 WHERE 岗位ID IN (${jobIdPlaceholders}) ORDER BY 显示顺序`,
  ).all(...jobIds);
  const allProjects = prepare(
    `SELECT 项目ID as project_id, 岗位ID as job_id, 项目标题 as title, 项目步骤 as steps FROM 岗位项目 WHERE 岗位ID IN (${jobIdPlaceholders})`,
  ).all(...jobIds);
  const allKnowledgeCards = prepare(
    `SELECT 卡片ID as card_id, 岗位ID as job_id, 术语 as term, 解释说明 as explanation, 显示顺序 as display_order FROM 岗位知识卡片 WHERE 岗位ID IN (${jobIdPlaceholders}) ORDER BY 显示顺序`,
  ).all(...jobIds);
